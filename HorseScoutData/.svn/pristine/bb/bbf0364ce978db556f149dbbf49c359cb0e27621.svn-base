...
//这里的location默认取的是默认的地址，进入该页面后重新复制定位的地址
Page({
  data: {
    imgUrl: app.url,
    locationAllId: {},
    ...
    objectArray1: [],
    index1: 0,
    array2: ['学校类型','幼儿园','小学','初中','高中'],
    objectArray2: [],
    index2: 0,
    array3: ['年级','一年级', '二年级', '三年级', '四年级','五年级','六年级'],
    index3: 0,
    array4: ['班级','一班', '二班', '三班', '四班', '五班', '六班', '七班', '八班', '九班', '十班','十一班','十二班','十三班','十四班','十五班','十六班','十七班','十八班','十九班','二十班','二十一班','二十二班','二十三班','二十四班','二十五班','二十六班','二十七班','二十八班','二十九班','三十班','三十一班','三十二班','三十三班','三十四班','三十五班','三十六班','三十七班','三十八班','三十九班','四十班','四十一班','四十二班','四十三班','四十四班','四十五班','四十六班','四十七班','四十八班','四十九班','五十班'],
    index4: 0,
    Kindergarten: ['年级','大班','中班','小班'],
    JuniorMiddleSchool: ['年级','初一','初二','初三'],
    highSchool: ['年级','高一','高二','高三'],
  ...
  overAll(e){
    ...
    console.log(wx.getStorageSync('haveIdCard'));
    wx.request({
      url: app.rQUrl + '/v1/childreninfo/addNewChildren',
      method: 'POST',
      header: { 'token': wx.getStorageSync('userInfo').oauthToken.token || '1' },
      data: {
        "birthday": wx.getStorageSync('ActiveDate') || '',
        "bmi": bmi,
        "childrenClass": Number(index4),
        ...
        "schoolId": wx.getStorageSync('schoolId') || '0' , 
        "schoolType": that.data.index2 == 0 ? "0" : that.data.index2 == 1 ? "1" : that.data.index2 == 2 ? "2" :that.data.index2 == 3 ? "3" : "4"  
      },
      success(res){
        if(res.data.code == '10000'){
          ...
          if(resData == "1"){
            ...
            wx.showToast({
              title: '该孩子已被添加，请前往绑定即可',
              icon:"none"
            })
          }


          wx.setStorageSync('oldRelationCode', wx.getStorageSync('relationCode'))
