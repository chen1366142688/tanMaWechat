...
Page({
  ...
  onLoad: function (options) {
    ...
    if(options.classId){
      this.setData({ height: res.windowHeight-55, options: options ,padding:20})
    }else{
      this.setData({ height: res.windowHeight - 55, options: options })
    }
...
function searchStudent(that,num) {
  wx.request({
    ...
    data: {
      ...
      "nickName":that.data.options.nickName ||  that.data.value,
    ...
    success(res) {
      ...
      if(res.data.code=='10000'){
        if (that.data.ismore) {//这是下拉刷新的
          ...
          if(res.data.response.length){
            that.setData({ studentList: res.data.response })
          }
        }
